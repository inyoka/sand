...
class Info():
    ...
    def calcStressScore(self):
        self.stressScore = (sum(self.finalScore.values())-self.finalScore.get('prosocial'))
    ...
    def outputResults(self):
        ...
        messageScrolled.insert(tk.INSERT, self.name.get())
        messageScrolled.insert(tk.INSERT, self.dob.get())
        messageScrolled.insert(tk.INSERT, (a.get() for a in self.incomplete))
    ...
    def fileSaveCSV(self):
        ...
        with name as f:
            ...
            w.writerow(['Stress score:']+[self.stressScore])
            w.writerows(self.finalScore.items())
            f.close()
